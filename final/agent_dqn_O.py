...
class AgentDQN:
    def __init__(self, env, eval):
        ...
        self.num_timesteps_for_each = 10000
    ...
    def make_action(self, state, test=False):
        ...
        if sample > eps_threshold:
            with torch.no_grad():
                action = self.online_net.forward(state.float()).max(1)[1].view(1, 1)
                return action.item()
        else:
...
if __name__ == '__main__':
    from agent_dqn_O import AgentDQN
    from env_NewPath_image_runV4 import Intestine_path
    env = Intestine_path()
