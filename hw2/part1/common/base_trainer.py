...
class BaseTrainer:
    ...
    def start(self):
        ...
        for epoch in bar:
            self._stat.reset()
            self._epoch = epoch + 1
            train_acc.append(self._run_epoch('train'))
            eval_acc.append(self._run_epoch('eval'))
            self._stat.log()
            self._save_ckpt()
    ...
    def _run_epoch(self, mode):
        ...
        for idx, batch in enumerate(bar):
            ...
            bar.set_postfix_str(str(self._stat[mode]['acc(label)']))
        bar.close()
        return self._stat[mode]['acc(label)']
