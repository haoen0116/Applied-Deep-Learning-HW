...
hair_mapping = ['hair_blonde', 'hair_orange', 'hair_brown', 'hair_black', 'hair_blue', 'hair_white']

eye_mapping = ['eye_brown', 'eye_blue', 'eye_green', 'eye_black']

face_mapping = ['face_African', 'face_Asian', 'face_Caucasian']
...
def generation_by_attributes(model, device, latent_dim, hair_classes, eye_classes, face_classes, glasses_classes,
    ...
    face_tag = torch.zeros(64, face_classes).to(device)
    glasses_tag = torch.zeros(64, glasses_classes).to(device)
    ...
    for i in range(64):
        hair_tag[i][hair_class], \
        eye_tag[i][eye_class], \
        face_tag[i][face_class], \
        glasses_tag[i][glasses_class] = 1, 1, 1, 1
    ...
    output = model(z, tag)
    if step is not None:
        file_path = '{} hair {} eyes {} faces {} glasses, step {}.png'.format(hair_mapping[hair_class], 
                                                                              eye_mapping[eye_class], 
                                                                              face_mapping[face_class], 
                                                                              glasses_mapping[glasses_class], 
                                                                              step)
    else:
        file_path = '{} hair {} eyes {} faces {} glasses.png'.format(hair_mapping[hair_class], 
                                                                 eye_mapping[eye_class],
                                                                 face_mapping[face_class], 
                                                                 glasses_mapping[glasses_class])
