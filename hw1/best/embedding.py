...
class Embedding:
    ...
    def __init__(self, embedding_path,
        ...
        if '</s>' not in self.word_dict:
            self.add('</s>', torch.zeros(self.get_dim()))
        if '<unk>' not in self.word_dict:
            self.add('<unk>')
        if '<spk1>' not in self.word_dict:
            self.add('<spk1>')
        if '<spk2>' not in self.word_dict:
            self.add('<spk2>')
        if '<url>' not in self.word_dict:
            self.add('<url>')
        if '<mail>' not in self.word_dict:
            self.add('<mail>')

    def save_word_dict(self):
        wordict = self.word_dict
        json_str = json.dumps(wordict)
        with open('wordict.json', 'w') as json_file:
            json_file.write(json_str)
        print("word_dict successfull save")
